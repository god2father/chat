var appUI = {
    ...
    makeChatContent:function(message,user){
		...
		if (message.attach && message.attach.type) {
			...
			var type = message.type,
                ...
                msgHtml
            var messageCustom = message.custom;//非官方自定义消息
            if(messageCustom!=undefined){
                try{//有无法正常转换json的消息进行错误处理
                    messageCustom =JSON.parse(message.custom);
                }
                catch(err){
                    message.custom='{}'
                }
            ...
            if(type==="tip"){
                ...
                if(messageCustom.type=="8"){
                    msgHtml = [
                        ...
                        '<div class=" msg-custom-keyword j-msg">',
                        '<div class="box">',
                        '<div class="cnt">',
                        getMessage(message),
                        '</div>',
                    ...
                    msgHtml = [
                        ...
                        message.status === "fail"?'<span class="error j-resend" data-session="'+ message.sessionId +'" data-id="'+ message.idClient +'"><i class="icon icon-error"></i>发送失败,点击重发</span>':'',
                        '</div>'].join('');
                }else if(messageCustom.type==null){
                    msgHtml = [
                        '<div data-time="'+ message.time +'" data-id="'+ message.idClient +'" id="'+ message.idClient +'" data-idServer="'+ message.idServer +'" class="item item-' + buildSender(message) + '">',
                        '<div class=" msg-custom-keyword j-msg">',
                        '<div class="box">',
                        ...
                        message.status === "fail"?'<span class="error j-resend" data-session="'+ message.sessionId +'" data-id="'+ message.idClient +'"><i class="icon icon-error"></i>发送失败,点击重发</span>':'',
                        '</div>'].join('');
                }else if(messageCustom.type==undefined){
                    msgHtml = [
                        '<div data-time="'+ message.time +'" data-id="'+ message.idClient +'" id="'+ message.idClient +'" data-idServer="'+ message.idServer +'" class="item item-' + buildSender(message) + '">',
                        '<div class=" msg-custom-keyword j-msg">',
                        '<div class="box">',
    ...
    makeTimeTag : function(time){
    	return '<p class="u-msgTime pt5 pb5">&nbsp;'+time+'&nbsp;</p>';
