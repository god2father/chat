...
YX.fn.showMyInfo = function () {
    ...
    $.ajax({
        ...
        success: function(data) {
            if(data.state=="success"){
                $('#myWechat').text(data.info.wechat ===undefined?"--":data.info.wechat||"--")
                $('#myQQ').text(data.info.qq ===undefined?"--":data.info.qq||"--")
                $('#myEmali').text(data.info.email ===undefined?"--":data.info.email||"--")
            }else {
    ...
    // // $node.find(".j-email").text(user.email ===undefined?"--":my_detail.info.email||"--")
    // $node.find(".j-email").text(my_detail.info.email )
        ...
        success: function(data) {
            if(data.state=="success"){
                if(data.info.gender=='1'){
                    data.info.gender='男'
                }else if(data.info.gender=='2'){
                    data.info.gender='女'
                }else {
                    data.info.gender='保密'
                }
                $('#userGender').text(data.info.gender)
                $('#userWeChat').text(data.info.wechat ===undefined?"--":data.info.wechat||"--")
