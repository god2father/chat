...
function getUserkey(uid,token,toid,anid) {
    var localUid=readCookie('uid')
    var localToken=readCookie('token')
    setCookie('anid',anid)
    if (uid==localUid){
        console.log('reload')
        setCookie('toId',toid);
        $('.chat-logo').trigger('click')//连接完成后触发打开聊天界面
    }else {
        console.log(uid)
        console.log(token)
        console.log(toid)
 ...
 * @param callback：回调
 */
SDKBridge.prototype.sendCustomMessage = function (scene, to, content ,custom, callback) {
 ...
 * @return {void}
 */
SDKBridge.prototype.deleteSession = function(scene, to,done){
