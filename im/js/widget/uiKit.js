(function webpackUniversalModuleDefinition(root, factory) {
    ...
    /******/ ([
        ...
        /***/ function(module, exports, __webpack_require__) {
             ...
             * @return {Void}
             */
            SessionList.prototype.delete = function(data){
                var i,
                    str,
                    info,
                    sessions = data.sessions;
                $('#sessions').delegate('.delete-session-btn','click',function () {
                    var html = ''
                    var num= $(this).attr('num')
                    console.log(sessions.length)
                    sessions.splice(num,1)

                    console.log(JSON.stringify(sessions[num].lastMsg+'length'+sessions.length))
                    if (sessions.length === 0) {
                        html += '<p class="empty">暂无最近联系人哦</p>';
                    }else{
                        for (i = 0;i<sessions.length;i++) {
                            // info =YX.fn.infoProvider(sessions[i],"session");
                            info =sessions[i];
                            if(!info){
                                continue;
                            }
                            alert(1)
                            str = ['<li class="panel_item '+(info.crtSession===info.target?'active':'')+'" data-scene="' + info.lastMsg.scene + '" data-account="' + info.lastMsg.account + '">',
                                '<div class="panel_avatar"><img class="panel_image" src="'+info.avatar+'"/></div>',
                                '<div class="panel_text">',
                                '<p class="panel_multi-row">',
                                '<span class="panel_nick">' +info.lastMsg.fromNick + '</span>',
                                '<b class="panel_time">' + info.lastMsg.time + '</b>',
                                '</p>',
                                '<p class="panel_multi-row">',
                                '<span class="panel_lastMsg">' + info.lastMsg.text + '</span>',
                                info.unread ? '<b class="panel_count">' + info.unread + '</b>':'',
                                '</p>',
                                '<a href="javascript:;"  class="delete-session-btn" num="'+i+'"><i class="fa fa-close"></i></a>',
                                '</div>',
                                '</li>'].join("");
                            html += str;
                        }
                    }
                    $('#sessions .j-session').html(html);
                })
            };
